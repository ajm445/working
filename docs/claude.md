...
워킹홀리데이를 하는 사람들을 위한 통합 관리 웹앱입니다. 해외 생활비 관리와 출국 전 초기비용 계산을 하나의 앱에서 제공합니다.

### 주요 기능
- ✅ **이중 모드 시스템**: 가계부와 초기비용 계산기 전환
- ✅ 수입/지출 내역 관리
...
│   │   ├── TransactionItem.tsx
│   │   └── index.ts
│   ├── InitialCostCalculator/ # 초기비용 계산기 모드
...
├── hooks/                   # 커스텀 훅
│   ├── useCurrency.ts
│   └── useCurrencyConversion.ts  # 기존 + 새로운 useCurrencyConverter
├── utils/                   # 유틸리티 함수
...
│   └── initialCost.ts      # 초기비용 관련 타입
├── data/                   # 정적 데이터
│   └── initialCostCategories.ts  # 초기비용 카테고리 데이터
└── constants/              # 상수
    └── routes.ts           # 라우트 상수
```
...
## 🌟 새로 추가된 주요 기능 (최신 업데이트)

### 1. 이중 모드 시스템 (NEW!)
- **가계부 모드**: 기존 수입/지출 관리 기능
- **초기비용 계산기 모드**: 워킹홀리데이 준비 비용 계산
- **모드 전환**: 상단 탭을 통한 직관적인 전환
- **상태 관리**: AppModeContext를 통한 전역 모드 상태 관리

### 2. 워킹홀리데이 초기비용 계산기 (NEW!)
- **지원 국가**: 호주, 캐나다, 뉴질랜드, 영국, 일본
- **비용 카테고리**:
  - **필수**: 항공료, 비자, 보험, 숙박
  - **선택적**: 어학원, 에이전트, 생활용품, 여행경비 등
- **실시간 계산**: 입력 시 즉시 총 비용 계산
- **통화 변환**: 모든 비용을 현재 선택된 통화로 표시
- **예상 범위**: 각 카테고리별 예상 비용 범위 제공
...
### 📝 향후 개발 권장사항
1. **데이터 지속성**: LocalStorage 구현으로 브라우저 새로고침 시에도 데이터 유지
2. **통계 및 분석**: 월별/주별 지출 분석 차트 및 카테고리별 통계
3. **초기비용 계산기 개선**:
   - 더 많은 국가 추가
   - 세부 비용 항목 확장
   - 예산 대비 실제 비용 비교 기능
4. **사용자 경험 개선**: 알림 시스템, 예산 설정 및 초과 경고 기능
5. **성능 최적화**: React.memo, useMemo, useCallback을 활용한 렌더링 최적화
6. **접근성 향상**: ARIA 라벨, 키보드 네비게이션 등 웹 접근성 표준 준수
7. **Git 저장소 초기화**: 버전 관리 시작 및 브랜치 전략 수립
...
*이 문서는 워킹홀리데이 가계부 React 웹앱 프로젝트의 개발 가이드입니다. Claude CLI와 개발자 간의 효율적인 협업을 위한 규약을 포함하고 있으며, 프로젝트 진행에 따라 업데이트됩니다.*